...
class TestCreatItemAPI:
    def test_create_with_correct_request(self, session, client, initial_items_count):
        ...
        outbox_events_count = session.execute(
        ...
        new_event_payload["created_at"] = datetime.fromisoformat(
            new_event_payload["created_at"],
        )

        for key, val in new_event_payload.items():
            assert hasattr(new_item, key)
            assert getattr(new_item, key) == val
